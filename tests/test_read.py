...
def test_read_file_exist(ssd):
    if os.path.exists(TARGET_FILE):
        os.remove(TARGET_FILE)
    assert not os.path.exists(TARGET_FILE)
    ssd.init_target_file()
    ssd.read(0)
    assert os.path.exists(TARGET_FILE)
    os.remove(TARGET_FILE)


def test_write_file_exist(ssd):
    if os.path.exists(TARGET_FILE):
        os.remove(TARGET_FILE)
    assert not os.path.exists(TARGET_FILE)
    ssd.init_target_file()
...
def test_read_success_lba0(ssd):
    ssd.init_target_file()
    write_file(0, TEST_VALUE)
    ssd.read(0)
...
def test_read_valid_value(ssd, valid_value):
    ssd.init_target_file()
...
def test_read_invalid_value(ssd, invalid_value):
    ssd.init_target_file()
    write_file(0, invalid_value)
    ssd.read(0)
    assert get_output_file() == ERROR_STRING

def test_read_success_lba99(ssd):
    ssd.init_target_file()
...
def test_read_blank_success(ssd):
    if os.path.exists(TARGET_FILE):
        os.remove(TARGET_FILE)
...
def test_read_out_of_range_lba_error(ssd, invalid_lba):
    ssd.init_target_file()
    ssd.read(invalid_lba)
    assert get_output_file() == ERROR_STRING

@pytest.mark.parametrize("invalid_lba", ['A', '10', '50', 'AAA'])
def test_read_invalid_lba_error(ssd, invalid_lba):
    ssd.init_target_file()
    ssd.read(invalid_lba)
    assert get_output_file() == ERROR_STRING
